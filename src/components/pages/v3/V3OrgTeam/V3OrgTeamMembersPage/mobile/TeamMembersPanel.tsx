...
export const TeamMembersPanel = memo((props: TeamMembersPanel) => {
    const {setIsShow} = useModal({isShowAtom: isOpenNewTeamMemberModalAtom});
    ...
    return (
        <MobileSection.Item>
            <MobileSection.Padding>
                <MobileSection.Heading title={'member'}>
                ...
                {length ? (
                    ...
                    <ContentEmpty text="등록된 멤버이 없어요" subtext="눌러서 멤버 추가" onClick={onAddButtonClick} />
                )}
            </MobileSection.Padding>
        </MobileSection.Item>
    );
