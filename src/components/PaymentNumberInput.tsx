...
export default function PaymentNumberInput({
  month,
  suffix,
  value,
}: PaymentNumberInputProps) {
  ...
  return (
    <div className="flex items-center justify-between rounded-md bg-[#F5F6F7] py-2 px-3">
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1 flex-col">
          <TextField
            className={`{w-full ${
              lock && "border-none"
            } disabled:bg-[#F5F6F7] disabled:text-black`}
            type="number"
            disabled={lock}
            value={value}
          />
      ...
      <button
        className={`${
          lock ? "outlined-gray-600 bg-white" : "filled-brand-1"
        } h-10 flex-shrink-0 px-5`}
