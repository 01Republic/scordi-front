...
export const OrgOnboardingMembersConnectsPageRoute = pathRoute({
    ...
    path: (id: number, connectorName: Connectors) =>
        pathReplace(OrgOnboardingMembersConnectsPageRoute.pathname, {
            id,
            connectorName,
        }),
});
...
export default function OrgConnectorDetailPage() {
    ...
    return (
        <div>
            <div>
                <p>redirect to: {OrgOnboardingMembersConnectsPageRoute.path(orgId, connectorName as Connectors)}</p>
            </div>
        </div>
    );
}
