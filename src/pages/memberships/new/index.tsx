    ...
    <OrgSettingsLayout>
      ...
      <OrgSettingsLayout.Body>
        <OrgSettingsLayout.Container>
          {results.length > 0 && results.map((org) => (
            <SearchResultItem
              key={org.id}
        ...
        <OrgSettingsLayout.Container>
          <div className="pt-[1rem] space-y-4">
            <button className="btn btn-primary btn-block btn-lg" type="button" disabled={!selectedOrg} onClick={startWithSelectedOrg}>{selectedOrg ? '이 조직으로 시작하기' : '시작할 조직을 선택해주세요'}</button>
            <button className="btn btn-outline btn-primary btn-block btn-lg" type="button" onClick={startWithNewOrg}>조직을 찾지 못하셨나요?&nbsp;&nbsp;&nbsp;&nbsp;새 조직 등록하기</button>
          </div>
        </OrgSettingsLayout.Container>
      </OrgSettingsLayout.Body>
    </OrgSettingsLayout>
