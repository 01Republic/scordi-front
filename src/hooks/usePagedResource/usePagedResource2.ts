...
export interface UsePagedResource2Option<DTO, Query, Dep extends any[]> {
    endpoint: (
        params: Query,
        orgId: number,
        subscriptionId: number,
        dep: Dep,
    ) => Promise<AxiosResponse<Paginated<DTO>>>;
    buildQuery?: (params: Query, orgId: number, subscriptionId: number) => Query;
    mergeMode?: boolean;
    getId: keyof DTO | ((dto: DTO) => any);
    useOrgId?: boolean;
    dependencies?: Dep;
 ...
 */
export function usePagedResource2<DTO, Query, Dep extends any[] = []>(
    atoms: PagedResourceAtoms<DTO, Query>,
    option: UsePagedResource2Option<DTO, Query, Dep>,
) {
    ...
    const search = useCallback(
        async (params: Query, mergeMode = defaultMergeMode, force = false) => {
            ...
            params = buildQuery(params, orgId, subscriptionId);
            const request = () => {
                __setIsLoading(true);
                return endpoint(params, orgId, subscriptionId, dependencies).finally(() => {
