        ...
        <section className="py-16 px-12 max-w-[1152px] mx-auto space-y-20 min-h-lvh">
            ...
            <section className={`flex items-center gap-2 mt-20`}>
                ...
                <div className="text-2xl font-bold">
                    총 <span className="text-primaryColor-900">{unitFormat(newMembers.length, '명')}</span>의 구성원을
                    불러왔어요!
                </div>
            </section>
            ...
            <section className="grid grid-cols-2 gap-3 mt-12">
                {newMembers.map((member, i) => (
                    ...
                    >
                        <Avatar src={member.profileImageUrl || undefined} className="w-10 h-10" />
            ...
            <section className="flex justify-center pb-12 mt-20">
                <button className={`btn btn-scordi btn-lg btn-wide`} onClick={onNext}>
                    다음
                </button>
            </section>
        </section>
...
interface ConnectingResultScreenForSlackProps {
    slackWorkspace: IntegrationWorkspaceDto<IntegrationProvider.slack>;
    onNext: () => void;
}

export const ConnectingResultScreenForSlack = memo((props: ConnectingResultScreenForSlackProps) => {
