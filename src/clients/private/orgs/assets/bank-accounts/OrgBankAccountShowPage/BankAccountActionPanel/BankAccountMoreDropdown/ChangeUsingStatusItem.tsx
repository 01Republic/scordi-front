...
export const ChangeUsingStatusItem = memo(function ChangeUsingStatusItem() {
    const {currentBankAccount, reload} = useCurrentBankAccount();
    const [isLoading, setIsLoading] = useState(false);

    if (!currentBankAccount) return <></>;

    const {usingStatus} = currentBankAccount;
                    ...
                    <Tippy
                        ...
                        render={() => (
                            <UsingStatusSelect
                                ...
                                selectOption={(option: BankAccountUsingStatus) => {
                                    if (!currentBankAccount) return;
                                    const {organizationId: orgId, id} = currentBankAccount;
                                    const req = bankAccountApi.update(orgId, id, {usingStatus: option});
...
const UsingStatusSelect = (props: {
    selectedValue: BankAccountUsingStatus;
    selectOption: (option: BankAccountUsingStatus) => any;
}) => {
