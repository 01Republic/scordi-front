...
export const getCodefAccountColumns = (options: Options): CardTableColumns<CodefAccountDto>[] => {
    ...
    const goCodefCardList = (account: CodefAccountDto) => moveTab(1, account);
    ...
    return [
        ...
        },
        {
            ...
            render: (account: CodefAccountDto) => (
                <div
                    ...
                    onClick={() => goCodefCardList(account)}
                >
                    {/* 카드사 */}
                    {account.company || '-'}
                </div>
        ...
        },
        {
            ...
            className: 'text-12',
            render: (account: CodefAccountDto) => (
                <div className="cursor-pointer" onClick={() => goCodefCardList(account)}>
        ...
        },
        {
            ...
            render: (account: CodefAccountDto) => {
                ...
                const syncCodefCardsOfAccount = () => {
                    const syncConfirm = () => confirm2('패치를 시작할까요?');
                    return confirmed(syncConfirm())
                        .then(() => codefAccountAdminApi.syncCards(orgId, account.id))
                        .then(() => toast.success('패치 완료'))
                        .then(() => goCodefCardList(account))
                        .catch(errorToast);
                };
                ...
                const removeAccount = () => {
                    const removeConfirm = () =>
                        confirm2(
                            ...
                            '이 계정으로 불러온 코드에프 카드와 결제내역은 모두 삭제되며, <br> 등록된 카드 데이터는 연동되지 않은 상태로 남아있게 됩니다.',
                        );
