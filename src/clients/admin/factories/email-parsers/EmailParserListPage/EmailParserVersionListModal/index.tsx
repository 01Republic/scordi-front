...
export const EmailParserVersionListModal = memo((props: EmailParserVersionListModalProps) => {
    ...
    const toggleActive = (e: ChangeEvent<HTMLInputElement>, parser: EmailParserDto) => {
    ...
    const remove = (parser: EmailParserDto) => {
        if (!confirm('Are you sure??')) return;

        setIsLoading(true);
        return gmailInvoiceParsersAdminApi
            .destroy(parser.id)
            .then(() => toast.success('Success!'))
            .then(() => reload && reload())
            .catch(errorToast)
